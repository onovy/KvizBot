...
/**
 * PircBotNg is a Java framework for writing IRC bots quickly and easily.
 *  <p>
 ...
 * channel statistics.
 *  <p>
 * Methods of the PircBotNg class can be called to send events to the IRC server
 ...
 * can be supported using multiple instances of PircBotNg.
 *  <p>
 * To perform an action when the PircBotNg receives a normal message from the IRC
 * server, you would override the onMessage method defined in the PircBotNg
 * class.  All on<i>XYZ</i> methods in the PircBotNg class are automatically called
 ...
 * methods are already correctly implemented in the PircBotNg and should not
 * normally need to be overridden.  Please read the full documentation for each
 * method to see which ones are already implemented by the PircBotNg class.
 *  <p>
 * Please visit the PircBotNg homepage at
 * <a href="http://pircbot-ng.nomi.cz/">http://pircbot-ng.nomi.cz/</a>
 * for full revision history, a beginners guide to creating your first PircBotNg
 * and a list of some existing Java IRC bots and clients that use the PircBotNg
 * framework.
 * 
 ...
 */
public abstract class PircBotNg implements ReplyConstants {


    /**
     * The definitive version number of this release of PircBotNg.
    ...
    /**
     * Constructs a PircBotNg with the default settings.  Your own constructors
     * in classes which extend the PircBotNg abstract class should be responsible
    ...
    public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
        connect(hostname, port, password, InetAddress.getLocalHost());
    }
     ...
     * @param port The port number to connect to on the server.
     * @param password The password to use to join the server.
     * @param localaddress An InetAddress indicating the local interface to
     * bind to when making the connection (the port is system-chosen).
     *
     * @throws IOException if it was not possible to connect to the server.
     * @throws IrcException if the server would not let us join it.
     * @throws NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname, int port, String password, InetAddress localaddress) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if (isConnected()) {
            throw new IOException("The PircBotNg is already connected to an IRC server.  Disconnect first.");
        }
        ...
        Socket socket = sf.createSocket(hostname, port, localaddress, 0) ;
     ...
     * quitServer() method.  Providing the PircBotNg was connected to an
     ...
     *  <p>
     * So when a PircBotNg is run on a machine that does not run an ident server,
     ...
     * When the PircBotNg receives information for each channel, it will
     ...
     * output if the PircBotNg has had its verbose mode turned on by calling
     * setVerbose(true);
     * 
     ...
     * then calling the appropriate method in the PircBotNg.  This method is
    ...
    protected void handleLine(String line) {
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            // If we reach this point, then we've found something that the PircBotNg
    ...
    /**
     * This method is called once the PircBotNg has successfully connected to
     * the IRC server.
     *  <p>
     * The implementation of this method in the PircBotNg abstract class
    ...
    /**
     * This method carries out the actions to be performed when the PircBotNg
     * gets disconnected.  This may happen if the PircBotNg quits from the
    ...
    /**
     * This method is called by the PircBotNg when a numeric response
     * is received from the IRC server.  We use this method to
     * allow PircBotNg to process various responses from the server
     ...
     * will be &quot;<code>PircBotNg #test :I Am King of Test</code>&quot;
     ...
     *  <p>
     * PircBotNg implements the interface ReplyConstants, which contains
     ...
     * users in that channel. The PircBotNg collects this information and
    ...
    /**
     * This method is called whenever a private message is sent to the PircBotNg.
     *  <p>
     * The implementation of this method in the PircBotNg abstract class
     * performs no actions and may be overridden as required.
     *
     ...
     * PircBotNg joins a new channel and discovers its topic.
     *  <p>
     * The implementation of this method in the PircBotNg abstract class
     * performs no actions and may be overridden as required.
     *
    ...
    /**
     * After calling the listChannels() method in PircBotNg, the server
    ...
    /**
     * This method used to be called when a DCC SEND request was sent to the PircBotNg.
    ...
    /**
     * This method used to be called when a DCC CHAT request was sent to the PircBotNg.
    ...
    /**
     * This method is called whenever a DCC SEND request is sent to the PircBotNg.
     ...
     * in order to allow multiple files to be downloaded by the PircBotNg
     * at the same time. 
     *  <p>
     ...
     * This abstract implementation responds with the PircBotNg's _version string,
     ...
     * the PircBotNg has not been programmed to recognise.
     *  <p>
     * The implementation of this method in the PircBotNg abstract class
     * performs no actions and may be overridden as required.
     * 
     ...
     * PircBotNg class in response to notification of nick changes that apply
     * to us.
     * 
    ...
    /**
     * Gets the name of the PircBotNg. This is the name that will be used as
     * as a nick when we try to join servers.
     *
     * @return The name of the PircBotNg.
     */
     ...
     *  <p>
     * The nick returned by this method is maintained only by the PircBotNg
    ...
    /**
     * Gets the internal login of the PircBotNg.
     *
     * @return The login of the PircBotNg.
     */
    ...
    /**
     * Gets the internal version of the PircBotNg.
     *
     * @return The version of the PircBotNg.
     */
    ...
    /**
     * Gets the internal finger message of the PircBotNg.
     *
     * @return The finger message of the PircBotNg.
     */
    ...
    /**
     * Returns whether or not the PircBotNg is currently connected to a server.
     * The result of this method should only act as a rough guide,
     * as the result may not be valid by the time you act upon it.
     *
     * @return True if and only if the PircBotNg is currently connected to a server.
     */
     ...
     * change this value in PircBotNg.  All lines greater than this length
    ...
    /**
     * Returns the name of the last IRC server the PircBotNg tried to connect to.
     * This does not imply that the connection attempt to the server was
     * successful (we suggest you look at the onConnect method).
     * A value of null is returned if the PircBotNg has never tried to connect
     * to a server.
     * 
    ...
    /**
     * Returns the port number of the last IRC server that the PircBotNg tried
     * to connect to.
     * This does not imply that the connection attempt to the server was
     * successful (we suggest you look at the onConnect method).
     * A value of -1 is returned if the PircBotNg has never tried to connect
     * to a server.
     * 
     * @since PircBot 0.9.9
     * 
     ...
     * "GB2312" for Chinese encoding.  If a PircBotNg is currently connected
     * to a server, then it must reconnect before this change takes effect.
     * 
     * @since PircBot 1.0.4
     * 
     * @param charset The new encoding charset to be used by PircBotNg.
     * 
    ...
    /**
     * Returns the InetAddress used by the PircBotNg.
     * This can be used to find the I.P. address from which the PircBotNg is
     ...
     * @since PircBot 1.4.4
     * 
     * @return An array of port numbers that PircBotNg can use to send DCC
     ...
     * @since PircBot 1.4.4
     * 
     * @param ports The set of port numbers that PircBotNg may use for DCC
     ...
     * same instance as this PircBotNg. This may be useful if you are writing
     * a multiple server IRC bot that uses more than one instance of PircBotNg.
     * 
     * @since PircBot 0.9.9
     *
     * @return true if and only if Object o is a PircBotNg and equal to this.
     */
    public boolean equals(Object o) {
        // This probably has the same effect as Object.equals, but that may change...
        if (o instanceof PircBotNg) {
            PircBotNg other = (PircBotNg) o;
            return other == this;
        }
        return false;
    }
    ...
    /**
     * Returns the hashCode of this PircBotNg. This method can be called by hashed
     * collection classes and is useful for managing multiple instances of
     * PircBotNgs in such collections.
     * 
     * @since PircBot 0.9.9
     * 
     * @return the hash code for this instance of PircBotNg.
     */
    public int hashCode() {
     ...
     * if you are using more than one PircBotNg instance to achieve
     * multiple server connectivity. The format of
     * this String may change between different versions of PircBotNg
     * but is currently something of the form
     * <code>
     *   Version{PircBotNg x.y.z Java IRC Bot - www.jibble.org}
     *   Connected{true}
     *   Server{irc.dal.net}
     *   Port{6667}
     *   Password{}
    ...
    /**
     * Disposes of all thread resources used by this PircBotNg. This may be
     * useful when writing bots or clients that use multiple servers (and
     * therefore multiple PircBotNg instances) or when integrating a PircBotNg
     * with an existing program.
     *  <p>
     * Each PircBotNg runs its own threads for dispatching messages from its
     * outgoing message queue and receiving messages from the server.
     * Calling dispose() ensures that these threads are
     * stopped, thus freeing up system resources and allowing the PircBotNg
     * object to be garbage collected if there are no other references to
     * it.
     *  <p>
     * Once a PircBotNg object has been disposed, it should not be used again.
     * Attempting to use a PircBotNg that has been disposed may result in
    ...
    // Default settings for the PircBotNg.
    private boolean _autoNickChange = false;
    private boolean _verbose = false;
    private String _name = "PircBotNg";
    private String _nick = _name;
    private String _login = "PircBotNg";
    private String _version = "PircBot-ng " + VERSION + " Java IRC Bot - pircbot-ng.nomi.cz";
